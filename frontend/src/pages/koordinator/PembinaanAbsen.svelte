<script lang="ts">
	...
	import type { AbsenRow, Pembinaan, User } from "@lib/types";
	...
	interface Props { user?: User; pembinaan: Pembinaan; absen?: AbsenRow[]; laporan?: LaporanAbsensi; flash?: { success?: string; error?: string }; }
	let { user, pembinaan, absen = [], laporan, flash }: Props = $props();
	...
	function save() {
		...
		saving = true;
		router.post(`/app/koordinator-guru/pembinaan/${pembinaan.id}/absen`, { absen: rows.map((r) => ({ guru_id: r.guru_id, hadir: r.hadir, jam_masuk: r.jam_masuk, keterangan: r.keterangan, alasan: r.alasan })) }, { preserveScroll: true, onFinish: () => (saving = false) });
	}
...
<AppLayout {user} group="koordinator-pembinaan">
	<div class="pt-8 pb-8 border-b border-neutral-200/80 dark:border-white/[0.04]">
		<div class="max-w-4xl mx-auto px-4 sm:px-6">
			<a href="/app/koordinator-guru/pembinaan" use:inertia class="inline-flex items-center gap-1.5 text-sm text-neutral-500 hover:text-brand-600 mb-4"><ArrowLeft class="w-4 h-4" /> Pembinaan</a>
			<h1 class="text-2xl font-bold text-neutral-900 dark:text-white">Absensi Pembinaan</h1>
			<p class="mt-1.5 text-neutral-600 dark:text-neutral-400">{pembinaan.tanggal} — {pembinaan.topik || "(tanpa topik)"}</p>
		</div>
	</div>
	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-4">
		...
		{#if laporan}
			<div class="flex justify-end gap-2 flex-wrap">
				<a href={`/app/koordinator-guru/pembinaan/${pembinaan.id}/absen/export`} class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-neutral-300 dark:border-neutral-700 text-sm font-semibold text-neutral-700 dark:text-neutral-200 hover:border-brand-400"><Download class="w-4 h-4" /> Export Excel</a>
